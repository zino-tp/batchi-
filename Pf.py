...
def get_saved_wifi_profiles_windows():
    ...
    for profile in profiles:
        ...
        # Get IP addresses for the profile
        ip_info = execute_command(f'netsh wlan show profile name="{profile}"')
        ip_info = ip_info.split('\n')
        ip_info = [line.strip().split(':') for line in ip_info if 'Profil aufgeführt' in line]

        for ip_line in ip_info:
            if ip_line[0] == 'Inhalt der IP-Konfiguration':
                ip_address = ip_line[1].split(':')[1].strip()
                profile_info['IP Address'] = ip_address
...
def get_saved_wifi_profiles_linux():
    ...
    for profile in profiles:
        ...
        # Get IP addresses for the profile (assuming IPv4 address)
        ip_info = execute_command(f'nmcli connection show {profile} | grep IP4.ADDRESS')
        ip_address = ip_info.split()[1] if ip_info else None
        profile_info['IP Address'] = ip_address
